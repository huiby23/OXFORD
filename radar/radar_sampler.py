"""
    Radar sampler classes for randomly (or sequentially) sampling batches of windows.
"""
...
class RandomWindowBatchSampler(Sampler):
    ...
    def __init__(self, batch_size, window_size, seq_lens, drop_last=True):
        ...
        for j, len_j in enumerate(seq_len_cumsum[1:]):
            valid_frames += list(range(seq_len_cumsum[j], len_j - batch_size * window_size + 1))
    ...
    def __iter__(self):
        ...
        for idx in self.sampler:
            batch += [idx + k * 1 for k in range(self.window_size * self.batch_size)]
