...
from utils.utils import draw_batch, plot_sequences
from utils.loss import supervised_loss_multi_score
...
class MonitorBase(object):
    ...
    def step(self, loss, dict_loss):
        ...
        if self.counter % self.config['log_rate'] == 0:
            ...
            # self.writer.add_scalar('train/match_temp', temperature, self.counter)
...
class SVDMonitor(MonitorBase):
    ...
    def validation(self):
        ...
        for batchi, batch in enumerate(self.valid_loader):
            ...
            loss, dict_loss = supervised_loss_multi_score(out['R'], out['t'], out['src'], out['tgt'], out['scores'], batch, self.config)
