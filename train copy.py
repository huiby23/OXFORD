...
def main():
    ...
    for epoch in range(args.total_epoch):
        ...
        for batch_idx, (im1, im2, pos_trans) in enumerate(tqdm(train_loader, desc=f"Training Epoch {epoch}", ncols=100)):
            ...
            # locations_map2, scores_map2, descriptors_map2 = model(im2)
            loss = pm_loss(locations_map1, scores_map1, descriptors_map1,locations_map2, scores_map2, descriptors_map2,pos_trans,alfa=10)
            loss.backward()
            optimizer.step()
            if batch_idx % 10 == 0:
        ...
        #     optimizer.zero_grad()
        #     locations_map1, scores_map1, descriptors_map1 = model(im1)
        #     locations_map2, scores_map2, descriptors_map2 = model(im2)
        #     loss = pm_loss(locations_map1, scores_map1, descriptors_map1,locations_map2, scores_map2, descriptors_map2,pos_trans,alfa=10)
        #     loss.backward()
        #     optimizer.step()
        ...
        #     for batch_idx, (inputs, masks) in enumerate(tqdm(val_loader, desc=f"Validating Epoch {epoch}", ncols=100)):
